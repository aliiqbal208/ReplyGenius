...
const debug = {
    ...
    log: (message, data = null) => {
        ...
        console.log(`[ReplyGenius] ${message}`, data || '');
    },
    ...
    error: (message, error = null) => {
        console.error(`[ReplyGenius] ${message}`, error || '');
    },
...
const TONE_CONFIG = {
    /**
     * Available tones grouped by category
     */
    CATEGORIES: {
        'Professional': [
    ...
    /**
     * Get all tones as a flat array
     * @returns {Array} Array of all tone objects
     */
    getAllTones: function() {
        const allTones = [];
        for (const category in this.CATEGORIES) {
            allTones.push(...this.CATEGORIES[category]);
        }
        return allTones;
    },
...
const GPT_MODELS = {
    ...
    /**
     * Get all models as a flat array (for backward compatibility)
     */
    MODELS: null, // Will be populated below
    
    /**
     * Get all models as a flat array
     * @returns {Array} Array of all model objects
     */
    getAllModels: function() {
        if (!this.MODELS) {
            const allModels = [];
            for (const category in this.CATEGORIES) {
                allModels.push(...this.CATEGORIES[category]);
            }
...
function createSettingsUI() {
    ...
    heading.textContent = 'ReplyGenius Settings';
...
function createCommentUI(post, generateButton) {
    ...
    // Add tone options - get from TONE_CONFIG
    const tones = TONE_CONFIG.getAllTones();
    ...
    // Add model options - get from GPT_MODELS
    const allModels = GPT_MODELS.getAllModels();
...
/**
 * Initializes the ReplyGenius extension
 * Sets up observers, keyboard shortcuts, and initial button placement
 */
function initialize() {
    debug.log('ReplyGenius initializing');
